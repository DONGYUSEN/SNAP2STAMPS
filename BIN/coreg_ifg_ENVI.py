...
graphxml=GRAPH+'/DEM-Generation-Pre-Processing-ENVI.xml'
print graphxml
graph2run=PROJECT+'/graphs/coreg_ifg2run.xml'
...
for dimfile in glob.iglob(slavesplittedfolder + '/*/*'+'.N1'):
    print dimfile
    k=k+1
    ...
    outputname=tailm[14:22]+'_'+tail[14:22]+'.dim'
    ...
    if process.returncode != 0 :
        ...
        stdout = process.communicate()[0]
        print 'SNAP STDOUT:{}'.format(stdout)
        print('['+str(k)+'] Finished reprocess in '+str((time.time() - timeStarted)/60.0)+' mins.')
    else:
    ...
    print bar_message
    out_file.write(bar_message)
    print('['+str(k)+'] Finished this process in '+str((time.time() - timeStarted)/60.0)+' mins.')
out_file.close()
